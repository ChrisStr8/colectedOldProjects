...
public class FastFood{
    ...
    private Queue<Order> drawnOrders;
    ...
    public void doKey(String key){
        if (keyToAction.get(key).equals("Deliver Order")){
            deliverOrder();
            drawOrders();
        } else{
            addItem(keyToAction.get(key));
            drawOrders();
        }
    }
    ...
    public void addItem(String item) {
        ...
        if (orders.peek()!=null){
            ...
            if (!order.addItemToOrder(item)){
                this.balance -= (double)(prices.get(item));
            }
        }
    }
    ...
    public void drawOrders() {
        UI.clearGraphics();
        /*# YOUR CODE HERE */
        UI.drawString("Balance: $"+balance,10,10);
        drawnOrders = new ArrayDeque<>();
        int y = 10;
        while (orders.peek()!=null){
            Order order = orders.poll();
            order.draw(y);
            drawnOrders.add(order);
            y += 40;
        }
        orders = drawnOrders;
    }
    ...
    private void initialiseMappings() {
        ...
        keyToAction.put("d", "Deliver Order");  keyToAction.put("D", "Deliver Order");
    }

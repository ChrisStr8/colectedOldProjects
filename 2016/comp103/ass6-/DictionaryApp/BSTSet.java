...
public class BSTSet <E extends Comparable<E>> extends AbstractSet <E> {
    ...
    public boolean contains(E item) throws ClassCastException {
        /*# YOUR CODE HERE */
        //if(root==null ||item==null){throw new ClassCastException();}
    ...
    public boolean add(E item) {
        ...
        if (!contains(item)) {
            count+=1;
            BSTNode<E> newNode = new BSTNode<>(item);
            if (root == null) {
                root = newNode;
                return true;
            }
    ...
    private class BSTSetIterator implements Iterator<E> {
        ...
        Stack<BSTNode<E>> nodeStack = new Stack<>();
        ...
        private void populateFrom(BSTNode<E>root){
            if(root!=null) {
                nodeStack.push(root);
                if (root.getLeft() != null) {
                    populateFrom(root.getLeft());
                }
                if (root.getRight() != null) {
                    populateFrom(root.getRight());
                }
            }
        }

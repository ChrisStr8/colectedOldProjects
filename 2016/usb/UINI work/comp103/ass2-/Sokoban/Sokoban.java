...
public class Sokoban {
    ...
    private Stack<ActionRecord> history;
    private Stack<ActionRecord> undoHistory;
    ...
    public void doKey(String key) {
        if (keyToAction.get(key).equals("undo")){
            undo();
        } else if (keyToAction.get(key).equals("redo")){
            redo();
        } else{
    ...
    public void move(String direction) {
        if (!undoing){this.history.push(new ActionRecord("move", direction));}
    ...
    public void push(String direction) {
        this.history.push(new ActionRecord("push", direction));
        drawSquare(workerPosition); // display square under worker
    ...
    /**undoes past actions stored in history*/
    public void undo(){
        if (!this.history.empty()){
            ActionRecord action = this.history.pop();
            String oppositeDir = oppositeDirection(action.direction());
            if (action.isPush()){
                undoing = true;
                pull(oppositeDir);
            } else{
    ...
    /**redoes undone actions stored in undoHistory*/
    public void redo(){
        if (!this.undoHistory.empty()){
            ActionRecord action = this.undoHistory.pop();
            if (action.isPush()){

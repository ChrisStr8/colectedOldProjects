...
public class GeneralTree
{
     ...
     *  
     *  @param newName the name of the node to be added
    ...
    public void addNode(String newName, String parentName) {
        ...
        if(parentNode!=null && !root.contains(newNode)){parentNode.addChild(newNode);}
        redraw();
    }
    ...
    public void removeNode(String targetName) {
        /*# YOUR CODE HERE */
        GeneralTreeNode node = findNode(targetName);
    ...
    public void moveSubtree(String targetName, String destinationName) {
        ...
        destinationNode.addChildrenFromNode(targetNode);
        targetNode.remove();
        redraw();
    }
    ...
    public String findClosestCommonAncestor(String target1, String target2) {
        /*# YOUR CODE HERE */
        GeneralTreeNode node = root.findNode(target1);
        while(node.getParent()!=null){
            node = node.getParent();
            if(node.findNode(target2)!=null){return node.getName();}
        }
        if(node.findNode(target2)!=null){return node.getName();}
        return null;
    }
    ...
    private void calculateLocations() {
        double drawingSpace = (UI.getCanvasWidth())*0.5;
        root.setLocation(new Location(drawingSpace, levelSep/2));
        root.setChildrenLocations(drawingSpace);
    }

...
public class ThreadChallenge {
    ...
    private final ArrayQueueCh<String> queue;

    public ThreadChallenge() {
        // Replace with your ArrayQueue
        // queue = new java.util.concurrent.ArrayBlockingQueue<>(NUM_PER_THREAD);
         //queue = new ArrayQueue<>();
        queue = new ArrayQueueCh<>();
    }
    ...
    public void run() throws InterruptedException {
        ...
        for (int i = 0; i < NUM_CONSUMERS; i++) {
            consumers.add(new Consumer(i + 1, (BlockingQueue<String>) queue, remainingValues));
        }
    ...
    private class Producer extends Thread {
        ...
        public Producer(int i, ArrayQueueCh<String> queue, Iterable<String> input) {
            super("Producer " + i);
            this.queue = queue;
            this.input = input;
        }

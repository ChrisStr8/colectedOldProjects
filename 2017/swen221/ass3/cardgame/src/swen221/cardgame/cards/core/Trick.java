...
public class Trick {
	...
	public void play(Player p, Card c) throws IllegalMove {
		if(p==null){
			throw new IllegalMove("null player");
		}
		if(c==null){
			throw new IllegalMove("null card");
		}
		if(!p.direction.equals(getNextToPlay())){
			throw new IllegalMove("Not player "+p.direction+"s turn");
		}
		if(!p.hand.contains(c)){
			throw new IllegalMove("Not player "+p.direction+"s card");
		}
		if(p.direction!=lead && c.suit()!=getCardPlayed(lead).suit() && p.hand.matches(getCardPlayed(lead).suit()).size()>0){
			throw new IllegalMove("Card does not follow suit");
		}
	...
	@Override
	public Trick clone(){
		Trick clone = new Trick(lead, trumps);

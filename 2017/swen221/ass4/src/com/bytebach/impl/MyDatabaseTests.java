...
@SuppressWarnings({"EmptyCatchBlock", "ConfusingArgumentToVarargsMethod", "unused", "Convert2Diamond", "StringConcatenationInLoop"})
...
public class MyDatabaseTests {
	...
	private static final Field[] FIELDS_3 = {
			new Field("ID",Field.Type.INTEGER,true),
			new Field("ID2",Field.Type.TEXT,true),
			new Field("TEXT",Field.Type.TEXT,false)
	};
	...
	@Test
	public void test26_ModifyField(){
		Database db = createDatabase();
		createTable(db, "table", FIELDS_1);
		try{
			db.table("table").fields().remove(0);
			fail("should not be able to remove fields");
		} catch(InvalidOperation e) {}
	}

	@Test
	public void test27_ModifyField(){
		Database db = createDatabase();
		createTable(db, "table", FIELDS_1);
		try{
			db.table("table").fields().add(new Field("new", Field.Type.INTEGER, false));
			fail("should not be able to add fields");
		} catch(InvalidOperation e) {}
	}

	@Test
	public void test28_ModifyField(){
		Database db = createDatabase();
		createTable(db, "table", FIELDS_1);
		try{
			db.table("table").fields().set(0, new Field("new", Field.Type.INTEGER, false));
			fail("should not be able to set fields");
		} catch(InvalidOperation e) {}
	}

	@Test
	public void test29_DeleteTable(){
		Database db = createDatabase();
		createTable(db, "table", FIELDS_1);
		db.deleteTable("table");
		if(db.table("table")!=null){
			fail("should remove table \"table\"");
		}
	}

	@Test
	public void test30_CascadingDeleteTable(){
		Database db = createDatabase();
		createTable(db, "table", FIELDS_1);
		createTable(db, "table2", FIELDS_2);
		db.deleteTable("table");
		if(db.table("table")!=null && db.table("table2")!=null){
			fail("should remove table with field referencing removed table");
		}
	}

	@Test
	public void test31_AddRowMultipleKeys() {
		Object[][] rawData = {{0, "Hello WOrld", "TEST1"}, {0, "Blah", "test2"}};
		Value[][] rows = toValues(rawData);
	...
	@Test
	public void test32_AddRowMultipleKeys() {
		Object[][] rawData = {{0, "Hello WOrld", "TEST1"}, {0, "Hello WOrld", "test2"}};
		Value[][] rows = toValues(rawData);
		...
		try {
			addRow(db,"table",rows[1]);
			fail("Shouldn't be able to add row with same key fields as another");
		} catch(InvalidOperation e) {}
	}

	@Test
	public void test33_DeleteNonExistentTable(){
		Database db = createDatabase();
		try {
			db.deleteTable("notTable");
			fail("should not try to delete table that does not exist");
		} catch (InvalidOperation e){}
	}

	@Test
	public void test34_RemoveNonExistentRow(){
		Database db = createDatabase();
		createTable(db, "table", FIELDS_1);
		try{
			removeRow2(db, "table", new IntegerValue(0));
			fail("should not try to delete row that does not exist");
		} catch (InvalidOperation e){}
	}

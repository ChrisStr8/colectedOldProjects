...
public class TestUtils {
	...
	public static void checkManualCoverage(String filename, String inputSequence, int pulseLength) throws IOException {
		...
		if (coverage.getBranchCoverage() < COVERAGE_TARGET) {
			// Indicates a fail
			System.out.println("===============================================");
			System.out.println(filename + " (" + String.format("%.2f", coverage.getInstructionCoverage())
					+ "% instructions, " + String.format("%.2f", coverage.getBranchCoverage()) + "% branches)");
			System.out.println("===============================================");
			printDisassembly(tinyBoy, coverage);
			fail("Branch coverage failed to meet target of " + COVERAGE_TARGET + "%");
		} else {
			printDisassembly(tinyBoy, coverage);
	 ...
	 * @return
	 */
	private static TinyBoyInputSequence createInputSequence(String input, int pulseLength) {
	 ...
	 * @throws IOException
	 */
	public static CoverageAnalysis computeCoverage(TinyBoyEmulator tinyBoy, String filename,
			int cycles, AutomatedTester.InputGenerator<?> generator) throws IOException {
		...
		return tester.run(10, cycles, COVERAGE_TARGET);
	}
	 ...
	 * @param tinyBoy
	 * @param The
	 *            set of reachable instructions. This is critical to determining
